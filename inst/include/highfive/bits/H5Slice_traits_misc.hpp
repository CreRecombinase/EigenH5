...
inline void SliceTraits<Derivate>::read(T &array) {
  ...
  auto ds = details::get_dataset(static_cast<const Derivate *>(this));
  bool isTranspose = ds.isTransposed();
  if(isTranspose){
    HDF5ErrMapper::ToException<DataSpaceException>("Transposed data has been deprecated");
  }
  ...
  details::data_converter<T> converter(array, mem_space,dt);
